...
| 6 | _pending_ | `engine/core/muninn.py` — 34 dispatch sites instrumented via 1-line `log_event("pipeline.cli.<cmd>.start", {})` after each `if/elif args.command == "<cmd>":` + 7-line import block. `muninn/_engine.py` is now a shim (post 2026-05-19) so a single edit on the canonical suffices. Inserted via in-place Python regex script (idempotent if re-run). | ~40 | `git revert <hash>` |
| 7 | _pending_ | `muninn/ui/cube_live.py` (reconstruct_start/end +2 events, 9-line import block) + `muninn/ui/terminal.py` (subprocess_start/end +2 events, 9-line import block). Scope narrowed: only 2 files instrumented (the real UI→engine call paths). main_window/search/command_palette/tree_view/navi are pure navigation, no engine call — skipped. | ~25 | `git revert <hash>` |
| 8 | _pending_ | `muninn/mcp/server.py` — 10 `_<name>_impl` handlers instrumented with 1-line `log_event("pipeline.mcp.<name>.begin", {})` AFTER each docstring (preserves __doc__) + 9-line import block. Inserted via in-place Python regex script (idempotent). Handlers: recall_local, recall_meta, recall_dual, runbook_list_sections, runbook_get, bugs_list, bugs_get, tree_get_root, tree_get_branch, tree_list_branches. | ~20 | `git revert <hash>` |
| 9 | _pending_ | sandbox at `/home/sky/Bureau/muninn-sandbox/` (NOT in MUNINN- repo): bumped `Dockerfile` (+11 Qt libs + xdotool + x11-apps + imagemagick + QT_QPA_PLATFORM env), bumped `entrypoint.sh` (`[mcp]` → `[mcp,ui]`, banner mentions DISPLAY), NEW `run-ui.sh` (X11 forward variant). `run.sh` kept headless for compat. Manual revert: see snapshot in chunk 9 commit message. | ~70 (in sandbox, not in repo) | hand-revert per snapshot below |
