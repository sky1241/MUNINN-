...
| 6 | _pending_ | `engine/core/muninn.py` + `muninn/_engine.py` (BUG-091 duplicate, must be mirrored) — 34 dispatch sites each, instrumented via 1-line `log_event("pipeline.cli.<cmd>.start", {})` after each `if/elif args.command == "<cmd>":` + 7-line import block in each file. Inserted via in-place Python regex script (idempotent if re-run). | ~80 | `git revert <hash>` |
