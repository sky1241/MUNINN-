...
| 6 | _pending_ | `engine/core/muninn.py` — 34 dispatch sites instrumented via 1-line `log_event("pipeline.cli.<cmd>.start", {})` after each `if/elif args.command == "<cmd>":` + 7-line import block. Inserted via in-place Python regex script (idempotent if re-run). | ~40 | `git revert <hash>` |
