"""
Test atomic cube size TARGET_TOKENS = 112 holds across files.

The 112 value is the empirical sweep winner from
docs/BENCHMARK_TARGET_TOKENS_SWEEP_2026-05-13.md, not a geometric
derivation. The historical comment "14 tokens × 8 faces" was wrong on
two counts: a cube has 6 faces (not 8) and God's Number is a
combinatorial bound that does not distribute over geometric elements.
14 × 8 is a mnemonic only (14 = GN QTM for 2×2×2, 8 = cubelets).

Verifies each cube is within tolerance [92, 132] and averages ~112,
and that subdivision stays roughly uniform (mean / 8 ≈ 14, where 8 is
just the divisor that gives a readable "tokens per cubelet" figure).
"""
...
def test_constants():
    """Verify constants match the empirical sweep winner."""
    assert TARGET_TOKENS == 112, f"TARGET_TOKENS should be 112 (empirical sweep winner), got {TARGET_TOKENS}"
...
def _cube_sizes(file_path: str, label: str):
    ...
    # 112 / 8 = 14 tokens per cubelet (8 = vertices/cubelets, not faces)
    tokens_per_cubelet = avg / 8
    ...
    print(f"  Tokens per cubelet (avg/8): {tokens_per_cubelet:.1f} (target: 14.0)")
    ...
    return {
        ...
        "tokens_per_cubelet": tokens_per_cubelet,
        "cubes": cubes,
        "sizes": sizes,
    }
...
def main():
    print("=" * 60)
    print("ATOMIC CUBE SIZE TEST")
    print(f"Target: {TARGET_TOKENS} tokens (empirical sweep winner)")
    ...
    for r in results:
        ...
        print(f"  {r['file']:30s}: {r['n_cubes']:4d} cubes, avg={r['avg_size']:.1f}, "
              f"cubelet={r['tokens_per_cubelet']:.1f}, in_tol={r['pct_in_tolerance']:.1f}%")
    ...
    if all_sizes:
        ...
        print(f"\n  GLOBAL: {total_cubes} cubes, avg={global_avg:.1f} tokens, "
              f"cubelet={global_avg/8:.1f} tok/cubelet, "
              f"in_tolerance={global_in_tol}/{len(all_sizes)} ({global_in_tol/len(all_sizes)*100:.1f}%)")

        # Verdict
        cubelet_ok = 13.0 <= global_avg / 8 <= 15.0
        tol_ok = (global_in_tol / len(all_sizes)) > 0.7
        print(f"\n  Cubelet target (14.0 ± 1.0): {'PASS' if cubelet_ok else 'FAIL'} ({global_avg/8:.1f})")
