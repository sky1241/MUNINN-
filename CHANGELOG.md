...
| E.6 hardened tests | `31baa04` | ✅ | 6 nouveaux tests forts (sdist real-build extraction, shlex parse systemd, doctor live run, env var clamp via reload, top_k bounds negative + huge) en remplacement (parallèle) des 5 plus dangereux grep-presence. **G.10 recount honest 2026-05-12 (nuit)** : sur les 36 weak tests initialement signalés par l'audit, le breakdown réel après inspection ligne-à-ligne est **16 REAL improvements + 6 MEDIUM + 3 truly WEAK** (le reste étant des faux positifs de l'agent grep). Le claim original ("36 weak tests à remplacer") était un overclaim — la dette technique réelle est ~3-9 tests. |
