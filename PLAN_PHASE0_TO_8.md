...
- [ ] **X1b** Purge secrets existants dans mycelium.db + meta_mycelium.db [S]
...
- [ ] **X6** Fix saturation loss — float pas int (count=1000 crashe a 1) [XS]
- [ ] **X7** Fix feed progress — ecrire APRES mycelium.save() pas avant [XS]
- [ ] **X8** Fix DB handle leak — fermer retour migrate_from_json() [XS]
- [ ] **X9** Fix access_count reset dans prune (branches immortalisees) [XS]
- [ ] **X10** Fix CAST qui casse l'index dans decay WHERE clause [XS]
- [ ] **X11** Fix L2/L3 order — L3 (phrases) AVANT L2 (fillers) [XS]
- [ ] **X12** Fix Bearer regex — minimum 20 chars pour eviter false positive prose [XS]
- [ ] **X13** Fix hex false positive L10 — word boundary \b[a-f0-9]{7,40}\b [XS]
