...
def _core_bible() -> list[BibleEntry]:
    ...
    entries.append(BibleEntry(
        ...
        regex_per_language={
            ...
            "go": r'(?:password|passwd|secret|apiKey|token)\s*(?::=|=)\s*"[^"]{4,}"',
            "java": r'(?:password|passwd|secret|apiKey|token)\s*=\s*"[^"]{4,}"',
            "ruby": r'(?:password|passwd|secret|api_key|token)\s*=\s*["\'][^"\']{4,}["\']',
            "php": r'(?:\$password|\$passwd|\$secret|\$api_key|\$token)\s*=\s*["\'][^"\']{4,}["\']',
            "c_cpp": r'(?:password|passwd|secret|api_key|token)\s*=\s*"[^"]{4,}"',
        },
    ...
    entries.append(BibleEntry(
        ...
        regex_per_language={
            "python": r'(?:AES\.new|Fernet|HMAC)\s*\(\s*b?["\'][^"\']{8,}["\']',
            "javascript": r'(?:createCipher|createHmac|createSign)\s*\([^,]+,\s*["\'][^"\']{8,}["\']',
            "go": r'(?:aes\.NewCipher|hmac\.New)\s*\(\s*\[\]byte\s*\(\s*"[^"]{8,}"\)',
            "java": r'(?:SecretKeySpec|IvParameterSpec)\s*\(\s*"[^"]{8,}"',
        },
