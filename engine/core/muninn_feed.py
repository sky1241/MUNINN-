...
def _log_sync_error(context: str, exc: BaseException) -> None:
    """Append a swallowed exception with traceback to the rotating hook log.

    CHUNK A8 (2026-05-08): delegates to engine/core/_hook_logger which
    rotates the file (1 MB max, 3 backups) and falls back to stderr when
    the file is unwritable, so the audit trail is never silently lost.
    """
    try:
        from _hook_logger import log_hook_event
        log_hook_event("muninn_feed", context, exc)
    except Exception:
        # Last-resort: write to stderr. Never re-raise (hooks must exit 0).
        try:
            sys.stderr.write(
                f"[MUNINN HOOK LOG fallback] {datetime.now().isoformat()} "
                f"[muninn_feed:{context}] {type(exc).__name__}: {exc}\n"
            )
            sys.stderr.flush()
        except Exception:
            pass
