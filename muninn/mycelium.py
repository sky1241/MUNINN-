...
class Mycelium:
    ...
    # BRICK 15 (2026-04-11): hard size cap + bounded BFS subgraph builder.
    # See engine/core/mycelium.py for full rationale (BUG-091 dual mirror).
    _ADJ_CACHE_HARD_LIMIT = 500_000

    def _build_adj_cache(self) -> dict:
        """Build and cache adjacency list from all edges. Hard cap of
        _ADJ_CACHE_HARD_LIMIT edges. Above this, returns empty dict and
        warns; callers must use _build_adj_subgraph(seeds, hops) instead."""
    ...
    def _build_adj_subgraph(self, seeds, hops: int = 2, fanout_cap: int = 64) -> tuple:
        ...
        Per-node SQL with `LIMIT fanout_cap` so hub seeds don't blow up
        the BFS. See engine/core/mycelium.py for full docstring."""
        adj = {}
        max_w = 0.0
        if self._db is None:
    ...
    def spread_activation(self, seeds: list[str], hops: int = None,
        ...
        # BRICK 15 (2026-04-11): bounded BFS subgraph instead of full
        # adjacency cache. See engine/core/mycelium.py for full rationale.
    ...
    def transitive_inference(self, concept: str, max_hops: int = 3,
        ...
        # BRICK 15: bounded BFS subgraph instead of full adjacency cache.
