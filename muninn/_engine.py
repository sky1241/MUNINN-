...
try:
    ...
    try:
        ...
        __version__ = "1.1.0"  # checkout fallback (sync with pyproject.toml)
except Exception:
    __version__ = "1.1.0"
...
try:
    ...
    # Chunk C.1 split (2026-05-11 nuit): install/cron/scrub moved to
    # dedicated sibling modules. Re-import via the package shims so
    # muninn.* surface is preserved (ProxyModule still exposes
    # install_hooks, install_cron, scrub_secrets, etc.).
    ...
    from muninn_tree import *    # noqa: F401,F403
    from muninn_feed import *    # noqa: F401,F403
    # C.1 split fallback (source-tree mode, no package context)
    from muninn_install import *  # noqa: F401,F403
    from muninn_install import (  # noqa: F401
        install_hooks, install_cron, _detect_init_system,
        _repos_registry_path, _load_repos_registry, _register_repo,
...
def _print_empty_repo_hint(cwd):
    ...
    in a dir without `.muninn/`. Pre-D.4 fell through to MUNINN_ROOT default
    which on a pip-installed wheel = site-packages/.muninn/ (RULE 1 violation).
    """
...
def main():
    ...
    # CHUNK MCP D.4 (2026-05-12): nargs="?" → `muninn` (no args) shows welcome
    # banner instead of argparse "required: command" error.
    parser.add_argument("command", nargs="?", choices=[
        ...
        "metrics",
        # 2026-05-14: mirror — wire Winter Tree Engine subcommand.
        "winter-tree",
    ])
    ...
    # users (typical post-`pip install muninn-memory` flow).
    ...
    # at boot. Pre-fix: A3 helper existed but was only called via `muninn-mem doctor`.
    ...
    if args.command == "uninstall":
        ...
        # CHUNK MCP F.1 (2026-05-12): mirror of engine/core/muninn.py.
        repo = Path(args.repo or args.file or ".").resolve()
        if not repo.exists():
            print(f"ERROR: path does not exist: {repo}", file=sys.stderr)
            sys.exit(1)
    ...
    if args.command == "status":
        ...
        if not _REPO_PATH:
            ...
            if (cwd / ".muninn").exists():
                ...
                # CHUNK MCP D.4 (2026-05-12): mirror of engine/core/muninn.py.
                # Empty repo → hint instead of silently using MUNINN_ROOT
                # (which on a pip-installed wheel = site-packages, RULE 1).
    ...
    if args.command == "cube":
        ...
        # cli_{scan,run,status,god} delegates (mirror engine/core/muninn.py).
